import { SyncProducts } from "../wailsjs/go/backend/App";
import { EventsOnMultiple } from "../wailsjs/runtime/runtime";
import { useState, useEffect, useRef } from "react";
import { Progress, ProgressProps, Button, Result, Card } from "antd";
import {
    SmileOutlined,
    CloseCircleOutlined,
...
function ProductSync() {
    ...
    const onImportEvent = (message: string) => {
      ...
      EventsOnMultiple("productUpdate", onImportEvent, 1);
    ...
    return (
        <div id="App">
        <Button onClick={syncProducts} disabled={syncButton}>
        Sync Products
      </Button>
...
{status ? (
        <Result
          ...
          title={status ? "Products Synced" : "Error Syncing Products"}
          icon={
            status ? (
