...
import { LoadingOutlined, SmileOutlined, CloseCircleOutlined } from '@ant-design/icons';
import { Spin, Result, Button, Progress , ProgressProps, Card, Flex  } from 'antd';
        ...
        <div id="App">
            ...
            {loading ? (
                <div style={{display: 'flex', justifyContent: 'center', flexDirection: 'column', marginTop: 20}}>
                    <Progress type="dashboard" percent={Number(progress.toFixed(1))} strokeColor={conicColors} />
                </div>
            ) : null}
            ...
            {status ? (
                ...
                icon={status ? <SmileOutlined  style={{ fontSize: 100 }} /> : <CloseCircleOutlined style={{ fontSize: 100 }} />}
                extra={[
                    <Button type="primary" key="console" onClick={()=>{setStatus(false), setLogs([])}}>
                    Go Home
                    </Button>,
                  ]}
              />
            ...
            <Card title="Logs" style={{ marginTop: 20, height :400, overflow: 'auto'}} styles={{ header: { position: 'sticky', top: 0, zIndex: 1, background: '#fff' } }} ref={cardRef}>
                {logs.map((log, index) => (
                    <p key={index}>{log}</p>
                ))}
            </Card>
